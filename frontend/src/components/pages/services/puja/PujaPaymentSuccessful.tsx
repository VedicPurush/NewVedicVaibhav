...
const PujaPaymentSuccessful = () => {
  useEffect(() => {
    const amount = Number(localStorage.getItem("lastPujaAmount")) || 0;
    const orderId = localStorage.getItem("lastPujaOrderId") || localStorage.getItem("bookingId") || "";

    /**
     * Meta deduplication id — must be byte-identical to the server's CAPI
     * `event_id`, or Meta counts the browser Purchase and the CAPI Purchase as
     * two separate sales.
     *
     * The server builds `pooja_purchase_` + `booking.transactionId`, and
     * `transactionId` is the merchantTransactionId (poojaBooking.controller:
     * `const txnId = merchantTransactionId` → saved as `transactionId: txnId`).
     *
     * `orderId` above cannot be used for this: `lastPujaOrderId` is written as
     * `verifyRes.data.bookingId || merchantTransactionId`, and the verify
     * endpoint returns the Mongo `_id` as bookingId — so it is normally the
     * `_id`, not the transaction id, and every Puja booking was double-counted.
     * Both payment pages already store merchantTransactionId, so read that.
     */
    const metaTxnId = localStorage.getItem("merchantTransactionId") || orderId;
    ...
    if (typeof window.gtag === "function") {
      ...
      window.gtag("event", "purchase", {
        ...
        value: toInr(amount),
        currency: "INR",
        items: [{ item_name: "Puja Booking", item_category: "Puja" }],
      });
    }
    if (typeof window.fbq === "function") {
      ...
      }, { eventID: `pooja_purchase_${metaTxnId}` });
    }
  }, []);
  ...
  const saveRatingAndReview = async (customReview: string) => {
    try {
      ...
      if (!pujaId || !userName || !userPhone) {
        message.error(
          "Missing pujaId or contact details. Review not submitted."
        );
        setErrorMessage("Missing pujaId or contact details.");
        return;
      }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-green-50 relative overflow-hidden">
      ...
      <div className="text-center px-4 md:px-0 w-full md:w-1/3 relative z-10">
        ...
        >
          Thank you for booking pooja with us!
        </motion.p>
            ...
            <>
              ...
              >
                <motion.button
                  ...
                  onClick={() => router.push("/profile?tab=pooja")}
                >
                  ✅ Check Your Pooja
                </motion.button>
                <motion.button
        ...
        <AnimatePresence>
          {!pauseRedirect && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ delay: 2.5, duration: 0.3 }}
              className="my-6 text-base font-medium text-gray-500 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-full inline-block"
            >
...
export default PujaPaymentSuccessful;
